...
| **Agent orchestration** | Triggers for 29 agents at v0.5 target. As of v0.5 PR 3: 28 shipped (18 core + Tier 1: `debugger`, `postgres-pro`, `sql-pro`, `python-pro`, `prompt-engineer`, `refactoring-specialist` + Tier 2: `react-specialist`, `docker-expert`, `documentation-engineer`, `accessibility-tester`). Final specialist `mcp-developer` lands in PR 4 opt-in. Core set: `code-reviewer`, `architect-review`, `security-auditor`, `workflow-orchestrator`, `design-bridge`, `backend-developer`, `frontend-developer`, plus infra/language specialists. Write-capable specialists ship `-readonly` variants for Zaude's read-only commands. | `agents/` |
