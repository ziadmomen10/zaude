...
The vault is a git-tracked directory of markdown files holding everything Zaude knows about your projects. **In v1** the vault is the source of truth — written by `/wrap` / `/ship`, read by the `SessionStart` hook. **In v2 (the signed-trace kernel) the source of truth is the signed trace (`.zaude/trace.jsonl`); the vault becomes a _generated projection_ of it** — `current-state.md` and `decisions.md` are regenerated by `zaude vault-sync` and must never be hand-edited (the kernel's `protect_vault_projection` gate blocks that on onboarded repos). On a v2 project use `/zwrap` / `/zvault-sync`, not `/wrap` / `/ship`.
